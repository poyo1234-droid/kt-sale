...
**「通年-記入以外前回と同じ」シートの親カテゴリ欠落バグは `sale処理-v51.xlsm` で修正済み。自社環境（NAS / 260731 データ）で第7章の全検証項目1〜8が合格。**
...
1. **v52 の実装** — `仕様書_通年-階層優先順位の統一.md`（仕様確定済み・実装未着手）
2. **F版（`v51F` / `v52F`）を作って取引先環境で確認** — `作業手順_v51_親カテゴリ欠落修正.md` フェーズ2
...
| `仕様書_通年-階層優先順位の統一.md` | 階層の優先順位を 子カテ>親カテ>メーカー に統一（v52・未着手） |
