...
1. **`25 nobu` で親カテ単位（②）のテスト** — ②は実データが0件だったため未検証。手順は仕様書 10.7
2. **F版（`v51F`）を作って取引先環境で確認** — `作業手順_v51_親カテゴリ欠落修正.md` フェーズ2
3. GitHub へ push（ローカルが先行している）
4. 仕様書8章の既知課題（非通年側の同種欠落、Module3/4 のバグ、出力マクロの性能）
