...
OUTPUT_DIR = os.path.join(
    ...
    "セール情報設定リスト202608089～202608219", "kintone出力", "0908")
...
FIELDS = ["$id", "メーカーコード", "メーカー名", "ブランド名", "親カテゴリーコード",
          ...
          "ブランド連絡先担当者名", "ブランド連絡先メールアドレス",
          "ブランド連絡先担当者名_CC_", "ブランド連絡先メールアドレス_CC_", "FA様担当者",
          "作成日時", FILE_FIELD]
SLEEP_SEC = 0.1
...
def main():
    ...
    for rec in records:
        ...
        for fi in files:
            ...
            rows.append({
                ...
                "ブランド連絡先担当者名(CC)": val(rec, "ブランド連絡先担当者名_CC_"),
                "ブランド連絡先メールアドレス(CC)": val(rec, "ブランド連絡先メールアドレス_CC_"),
    ...
    cc_filled = sum(1 for r in rows if r["ブランド連絡先メールアドレス(CC)"])
    print("\n完了: %d 件DL / %d 件スキップ / %d 件エラー / 添付なし %d 件" % (ok, skip, err, nofile))
    print("宛先（メール・FA様担当者）が両方とも空: %d 件" % blank)
    print("CC（取引先）のメールアドレスが入っている件数: %d 件" % cc_filled)
