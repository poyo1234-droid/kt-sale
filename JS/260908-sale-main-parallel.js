...
async function executeSaleSettingsDataFetch(updateStatus = () => {}) {
  try {
    ...
    let allActiveProducts = await fetchRecordsFromApp(
    ...
    // 【高速化パッチ・実験用】対象メーカーを絞り込む。空配列なら絞り込みなし（全メーカー対象）。
    // 少数メーカーでの速度実験用。本番投入前にこのブロックごと削除すること。
    const TEST_LIMIT_MAKER_IDS = []; // 例: ['2', '9']
    if (TEST_LIMIT_MAKER_IDS.length > 0) {
      const beforeCount = allActiveProducts.length;
      allActiveProducts = allActiveProducts.filter(p => {
        const mId = (p['メーカーID']?.value || p['登録メーカーID']?.value || '')?.trim()?.replace(/\r?\n/g, '');
        return TEST_LIMIT_MAKER_IDS.includes(mId);
      });
      console.log(`【テスト絞り込み】商品マスタを ${beforeCount}件 → ${allActiveProducts.length}件 に絞り込みました（対象メーカー: ${TEST_LIMIT_MAKER_IDS.join(',')}）`);
    }
    ...
    // ----------------------------------------------------
    // 【高速化パッチ】(A)〜(E) は互いに依存しないため Promise.all で並列に取得する。
    // fetchRecordsInChunks も内部でチャンクを並列化した fetchRecordsInChunksParallel に変更。
    // ----------------------------------------------------
    updateStatus('在庫・在庫分析・salegoods・マスタ類・売上を並列取得中...');
    ...
    const [
      ...
      fetchRecordsInChunksParallel(clientInv, config['在庫'].id, csCodes, 'CS品番'),
      fetchRecordsInChunksParallel(clientAnalysis, config['在庫分析'].id, csCodes, 'CS品番'),
      fetchRecordsInChunksParallel(clientSale, config['salegoods'].id, brandCodes, 'ブランド品番'),
      loadManufacturers(),
      loadParentCategories(),
      clientSubCategory.record.getAllRecords({ app: config['子カテゴリマスタ'].id }),
      clientZozoType.record.getAllRecords({ app: config['ZOZO商品タイプマスタ'].id }),
      clientSales.record.getAllRecords({ app: config['Z_売上（注文日）'].id })
    ]);
    ...
    // 【高速化パッチ】グループごとの処理を関数化し、ワーカープールで並列実行する。
    // 中身のロジックは元のforループの中身と同一（データ抽出・Sheets/Drive/kintone呼び出し・リトライ）。
    async function processGroup(groupKey, i) {
      ...
      while (attempt < maxRetries && !isSuccess) {
        ...
        try {
          ...
          // 【高速化パッチ】並列実行でも登録順が元のグループ順と同じになるよう、インデックス位置へ格納する
          recordsToRegister[i] = recordData;
          
          isSuccess = true;
          successCount++;
          console.log(`--- [セール設定処理準備完了] ${progressLabel} ---`);
          
          // 【高速化パッチ】並列実行のため待機を短縮（1.0秒→0.3秒）。429が出た場合は fetchWithRetryOnceSale 側のバックオフに任せる
    ...
    // 【高速化パッチ】グループを GROUP_CONCURRENCY 並列のワーカープールで処理する。
    // まずは少数並列（3）で試し、レート制限（429）が出ないか確認してから増やす。
    const GROUP_CONCURRENCY = 3;
    let nextGroupIndex = 0;
    async function groupWorker() {
      while (nextGroupIndex < totalGroups) {
        const idx = nextGroupIndex++;
        await processGroup(groupKeys[idx], idx);
      }
    }
    await Promise.all(
      Array.from({ length: Math.min(GROUP_CONCURRENCY, totalGroups) }, () => groupWorker())
    );

    // ----------------------------------------------------
    // 4. kintone へのレコード一括登録
    // ----------------------------------------------------
    // 【高速化パッチ】インデックス位置に格納した配列から歯抜け（出力スキップ分）を除いて詰める
    const orderedRecordsToRegister = recordsToRegister.filter(r => r);
    if (orderedRecordsToRegister.length > 0) {
      updateStatus(`kintone へレコードを一括登録中（全 ${orderedRecordsToRegister.length} 件）...`);
      const appId = config['セール返答表'].id;
      const registeredIds = await registerRecordsInBulk(appId, orderedRecordsToRegister);
...
async function updateSpreadsheetValuesBatch(accessToken, spreadsheetId, valueRanges) {
  ...
  // 【高速化パッチ】並列実行で429が出やすくなるため、バックオフ付きの fetchWithRetryOnceSale に寄せる
  // （元は fetchWithRetryOnce。GoogleAPIFunctions.js 側にあり、429時の挙動が未確認だった）
  const response = await fetchWithRetryOnceSale(url, {
    method: 'POST',
    headers: {
...
/**
 * 【高速化パッチ】fetchRecordsInChunks の並列版。
 * チャンクを CHUNK_CONCURRENCY 個のワーカーで同時に処理する（既定5並列）。
 * kintoneのレート制限に当たったら数を減らす。
 */
async function fetchRecordsInChunksParallel(client, appId, codesList, fieldName, baseCondition = '', concurrency = 5) {
  if (!codesList || codesList.length === 0) return [];

  const chunkSize = 500;
  const chunks = [];
  for (let i = 0; i < codesList.length; i += chunkSize) {
    chunks.push(codesList.slice(i, i + chunkSize));
  }

  // 並列で取得しても結果の並び順が直列版と同じになるよう、チャンク番号の位置に格納する。
  // （配列末尾への push(...records) はレコード数が多いとスタック超過の恐れもあるため避ける）
  ...
  async function chunkWorker() {
    while (nextChunkIndex < chunks.length) {
      ...
      console.log(`【分割取得・並列】appId: ${appId}, チャンク ${idx + 1}/${chunks.length}（${chunk.length}件）取得中...`);
      const records = await client.record.getAllRecords({
        app: appId,
        condition: query
      });
  ...
  const workers = Array.from({ length: Math.min(concurrency, chunks.length) }, () => chunkWorker());
